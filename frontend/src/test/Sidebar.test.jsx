...
describe("Sidebar", () => {
  ...
  test("links every community module to its route", () => {
    ...
    expect(within(nav).getByRole("link", { name: "Estudiantes" })).toHaveAttribute(
      "href",
      "/app/alumnos"
    );
    ...
    expect(within(nav).getByRole("link", { name: "Padres de familia" })).toHaveAttribute(
      "href",
  ...
  test("links every academic-structure module to its route", () => {
    renderWithRouter(<Sidebar user={USER} />);
    const nav = screen.getByRole("navigation", { name: "Estructura academica" });
  ...
  test("links the daily-operation and control modules added with their backends", () => {
    ...
    const operation = screen.getByRole("navigation", { name: "Operacion diaria" });
    for (const [label, href] of [
      ["Matriculas", "/app/matriculas"],
      ["Asistencia", "/app/asistencia"],
      ["Evaluacion", "/app/evaluacion"],
    ]) {
      expect(within(operation).getByRole("link", { name: label })).toHaveAttribute("href", href);
    }

    const control = screen.getByRole("navigation", { name: "Documentos y control" });
    expect(within(control).getByRole("link", { name: "Plantillas" })).toHaveAttribute(
      "href",
      "/app/plantillas"
    );
    expect(within(control).getByRole("link", { name: "Alertas" })).toHaveAttribute(
      "href",
      "/app/alertas"
    );
  });
  ...
  test("shows the count badge only on the modules that report one", () => {
    renderWithRouter(<Sidebar counts={{ alumnos: 8, docentes: 0 }} user={USER} />);

    expect(screen.getByRole("link", { name: /Estudiantes/ })).toHaveTextContent("8");
    expect(screen.getByRole("link", { name: /Docentes/ })).toHaveTextContent("0");
    // Personas no declara `countable`, asi que no lleva insignia.
    expect(screen.getByRole("link", { name: /Personas/ })).not.toHaveTextContent(/\d/);
  });
