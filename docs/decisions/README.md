# Architecture Decision Records

## Objetivo

Registrar decisiones base y condiciones para revisarlas cuando requerimientos cambien.

## ADR disponibles

- [ADR-0001 Monorepository](ADR-0001-monorepository.md)
- [ADR-0002 Modular Monolith](ADR-0002-modular-monolith.md)
- [ADR-0003 Database Strategy](ADR-0003-database-strategy.md)
- [ADR-0004 REST API](ADR-0004-rest-api.md)
- [ADR-0005 Authentication and Authorization](ADR-0005-authentication-and-authorization.md)
- [ADR-0006 Soft Delete and History](ADR-0006-soft-delete-and-history.md)
- [Pending Decisions](pending-decisions.md)
