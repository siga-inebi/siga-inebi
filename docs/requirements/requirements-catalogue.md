...
| RF-ARC-006 | Retencion de adjuntos de justificacion | Deberia | document-management | Implemented | #83 | backend/tests/unit/test_attendance_services.py; backend/apps/attendance/services.py | Declaracion de retencion por adjunto con plazo configurable, sin purga automatica ni borrado fisico |
...
| RF-AUL-004 | Capacidad del aula como advertencia | Podria | institutional-structure | Implemented | #102 | backend/tests/unit/test_academics_services.py; backend/tests/api/test_academics_api.py; backend/tests/integration/test_academics.py | Advertencia derivada y no bloqueante para aula habitual y aula de sesion |
...
| RNF-CAP-002 | Crecimiento de almacenamiento estimado en 2 GB por ciclo, con umbral configurable y advertencia | Debe | file-storage | Implemented | #268 | backend/tests/unit/test_documents_services.py; backend/tests/api/test_documents_api.py | Alcance definido: calculo de consumo, proyeccion por ciclo y advertencia operativa en metadatos, sin purga ni eliminacion automatica; la recomendacion de almacenamiento es de monitorizacion y alerta, no de borrado ni compresion |
