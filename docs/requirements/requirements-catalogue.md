...
| RF-HOR-011 | Clonacion del horario | Podria | institutional-structure | Implemented | #204 | backend/tests/unit/test_academics_services.py; backend/tests/api/test_academics_api.py; backend/tests/integration/test_academics.py | Clonacion atomica entre secciones del mismo ciclo; compatibilidad por plan y bloques equivalentes |
...
| RF-BIT-004 | Registro de intentos denegados | Deberia | audit-compliance | Implemented | #114 | backend/tests/permissions/test_audit_permissions.py; backend/tests/api/test_audit_api.py | RNF exige varios casos; login fallido y autoescalamiento ya se auditaban, unico hueco real cerrado en `authorized_student_queryset` (apps/identity/scopes.py), choke point compartido por students/attendance/reporting |
...
| RF-AUT-003 | Duracion de sesion configurable por rol | Deberia | identity-access | In Progress | #107 | backend/apps/identity/{models.py,middleware.py,services.py}; backend/config/settings/base.py; frontend/src/{auth/AuthProvider.jsx,shared/api/apiClient.js} | Timeout por rol vigente y redireccion sin tokens en memoria persistente |
| RF-AUT-004 | Cierre de sesion | Debe | identity-access | In Progress | #108 | backend/{config/api/views.py,apps/identity/{services.py,api/}}; frontend/src/{auth/authService.js,shared/layout/UserMenu.jsx,auth/AccountsPage.jsx} | Cierre actual, global y administrativo con confirmacion y auditoria |
...
| RF-CTA-002 | Vinculacion de la cuenta a una persona registrada | Debe | people-registry | In Progress | #140 | backend/apps/identity/{models.py,services.py,api/serializers.py,migrations/0009_role_session_timeout_and_account_person_required.py} | La FK no admite cuentas huerfanas; provision de encargado valida perfil registrado |
...
| RF-EXP-001 | Registro del estudiante | Debe | student-records | Implemented | #185 | backend/tests/api/test_students_api.py; backend/tests/integration/test_student_lifecycle.py; frontend/src/test/AlumnosPage.test.jsx | Registro transaccional y autorizado con los campos institucionales ya definidos; no se agregan campos demograficos nuevos |
...
| RF-EXP-003 | Estado del estudiante | Debe | student-records | Implemented | #187 | backend/tests/unit/test_students_services.py; backend/tests/api/test_students_api.py; backend/tests/unit/test_enrolments_services.py | Solo active permite interaccion ordinaria; pre_enrolled habilita matricula/rematricula; withdrawn, graduated y transferred conservan desercion, finalizacion y traslado sin confundirlos |
| RF-EXP-005 | Contactos de emergencia | Debe | student-records | Implemented | #189 | backend/tests/api/test_emergency_contacts_api.py; backend/tests/unit/test_emergency_contact_services.py; frontend/src/test/AlumnosPage.test.jsx | Dato Restricted; flujo full-stack con permiso sensible, alcance obligatorio y auditoria de lectura y escritura |
...
| RF-EXP-007 | Fotografia del estudiante | Debe | student-records | Implemented | #191 | backend/tests/unit/test_students_services.py; backend/tests/api/test_students_api.py; frontend/src/test/AlumnosPage.test.jsx | Carga y visualizacion restringidas; maximo 5 MB, validacion real y normalizacion Pillow a JPEG 295 x 354 |
| RF-EXP-008 | Conservacion del expediente | Debe | student-records | Implemented | #192 | backend/tests/unit/test_students.py; backend/tests/api/test_students_api.py; backend/tests/integration/test_student_lifecycle.py | Baja logica desde API y bloqueo de borrado fisico por instancia o queryset para todo el expediente |
...
| RNF-CAP-001 | Dimensionamiento sobre la matricula real del establecimiento; toda meta se mide contra 1 vCPU y 2 GB | Debe | platform | Implemented | #267 | backend/tests/unit/test_platform_capacity.py | `CONN_MAX_AGE` reutiliza conexiones bajo el perfil 1 vCPU/2GB. Matricula de referencia ~500 estudiantes es una estimacion documentada en `docs/architecture/system-context.md`, pendiente de confirmar contra la matricula real |
...
| RNF-DIS-001 | Servicio disponible durante la ventana de jornada en dias lectivos; no se compromete operacion continua | Debe | platform | Implemented | #273 | N/A (documental) | Acuerdo operativo documentado en `docs/architecture/system-context.md`; ventana de jornada, sin 24/7, healthcheck de `compose.yml` como senal minima |
...
| RNF-OPE-001 | Registro de errores y monitoreo minimo del proceso trabajador y de las tareas programadas | Debe | platform | Implemented | #279 | TBD | `common.TaskRun` mas logger `siga.tasks`; contrato en docs/architecture/operations-monitoring.md. El worker sigue diferido por ADR-0009 |
...
| RNF-PRI-005 | Revelacion minima en la verificacion publica de documentos: tipo, folio, fecha y vigencia | Debe | document-generation | Implemented | #284 | backend/tests/unit/test_documents_services.py, backend/tests/api/test_documents_api.py | Consulta publica con metadatos minimos: tipo, folio, fecha y vigencia (`vigente`/`revocada`) sin exponer identidad del estudiante. |
| RNF-REN-001 | Percentil 95 de la confirmacion de escaneo en 2 s o menos, sobre la infraestructura objetivo | Debe | attendance-capture | Implemented | #285 | backend/tests/integration/test_attendance_performance.py | Metica clave; medicion es evidencia direccional en maquina de CI/desarrollo, no en el perfil objetivo 1 vCPU/2GB (ver RNF-CAP-001, #267) |
| RNF-REN-002 | Capacidad de pico del porton segun operadores concurrentes y tasa por operador | Debe | attendance-capture | Implemented | #286 | backend/tests/integration/test_attendance_performance.py | Cifra de 3 operadores concurrentes es una estimacion de referencia, pendiente de confirmar contra medicion real de sitio |
...
| RNF-RES-001 | El respaldo de la base de datos es independiente del de archivos y se restaura en la infraestructura objetivo | Debe | platform | Implemented | #289 | TBD | Dos pilas independientes con manifiesto y verificacion; docs/architecture/backup-and-recovery.md |
| RNF-RES-002 | Punto y tiempo objetivo de recuperacion declarados y probados antes de la entrega | Debe | platform | Implemented | #290 | TBD | RPO 24 h y RTO 4 h declarados y probados (PD-002); valores de referencia a confirmar con Direccion |
...
| RNF-SEG-005 | Las descargas se resuelven con enlaces de vigencia breve atados al portador; sin rutas directas a archivos | Debe | document-management | Implemented | #296 | backend/apps/documents/services.py, backend/apps/documents/api/views.py, backend/tests/unit/test_documents_services.py, backend/tests/api/test_documents_api.py | Seguridad |
