...
def test_update_section_normalises_the_new_name():
    section = SectionFactory(name="A")

    updated = update_section(section=section, name=" b ")

    assert updated.name == "B"


def test_update_section_renaming_to_its_own_name_is_allowed():
    section = SectionFactory(name="A")

    updated = update_section(section=section, name="a")

    assert updated.name == "A"


def test_update_section_rejects_blank_name():
    section = SectionFactory(name="A")

    with pytest.raises(DomainError, match="name"):
        update_section(section=section, name="   ")


def test_update_section_can_become_uncapped():
    section = SectionFactory(capacity=30)

    updated = update_section(section=section, capacity=0)

    assert updated.capacity == 0
    assert updated.available_seats is None
