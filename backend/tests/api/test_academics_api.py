...
def test_create_class_session_api_rejects_classroom_double_booked_in_the_same_slot(
    ...
    """RF-HOR-005: cruce por aula, secciones distintas."""
    cycle = AcademicCycleFactory(institution=institution)
    ...
    ClassSessionFactory(section=section_a, schedule_block=block, classroom=classroom, day_of_week=1)
    ...
    assert response.status_code == 400
    assert "El aula ya tiene otra sesion" in response.json()["error"]["detail"]
